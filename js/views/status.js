...
function RowTable(obj = {}, fmt = (k, v) => `${v}`) {
  ...
  return h("div", { class: "kvgrid", style: "margin-top:6px" },
    entries.flatMap(([k, v]) => [
      h("div", { class: "label" }, k),
...
export default function StatusView({
  ...
  const s = selId ? (statusMap?.[selId] || {}) : {}; // safe fallback
  ...
  // This uses your older global config. If not present, we simply hide the block.
  const overrides = loadOverrides();
  const effProfile = (selId && fertilityMap)
    ? mergedProfile(fertilityMap, overrides, selId)
    : null;
  ...
  return h("div", null, [
    ...
    selChar && h("div", { class: "grid", style: "margin-top:12px" }, [
      // ===== Left card: Classic sheet + Fertility Sim (as before) =====
      h("div", { class: "card" }, [
        selChar.portrait
